...
def print_results(data_path):
    ...
    mean_acc = df['strict_correct'].mean()
    mean_soft_acc = df['soft_correct'].mean()
    # Box accuracy
    #print(f"\nResults for || {desc_mode} || {model} || {segmentation_model}")
    print(f"Results for {data_path}")
    print(f"Strict accuracy = {mean_acc}")
    print(f"Soft accuracy = {mean_soft_acc}")
    return None
...
def main(): 
   ...
   base_path = "/home/iliab/rice/final_project/clean_notebooks/clean_resutls/final/gqa/"
